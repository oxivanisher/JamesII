    ...
    def finished_loop(self):
        for torrent_id in self.tr_conn.get_files():
            ...
            if torrent.isFinished and torrent.status == 'stopped':
                newname = torrent.name.replace(".", " ").replace(",", " ").replace("-", " ").replace("eztv", "").replace("ettv", "").replace("HDTV", "").replace("x264", "").replace("ASAP", "").replace("PROPER", "").replace("EVOLVE", "").replace("HD", "").replace("FQM", "").replace("LOL", "").replace("SiON", "").replace("2HD", "").replace("mp4", "").replace("avi", "").replace("mpg", "").replace("iso", "").replace("mkv", "")
                message = self.core.new_message(self.name)
                message.level = 2
