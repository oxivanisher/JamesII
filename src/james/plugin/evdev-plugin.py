...
class EvdevThread(PluginThread):
    ...
    def __init__(self, plugin, evdev_path, evdev_name):
        ...
        self.logger.debug(f"EVDEV Device: {evdev_path}")
        self.name = f"{self.name} > Device: {evdev_name} - {evdev_path}"
    ...
    def work(self):
        ...
        try:
            ...
            self.logger.warning(f'EVDEV Plugin could not be loaded. Retrying in 5 seconds. {e}')
            time.sleep(5)
            self.work()

    def on_exit(self, result=0):
        self.logger.info(f'Exited with ({result})')
...
class EvdevPlugin(Plugin):
    ...
    def get_all_devices(self):
        ...
        for device in devices:
            self.logger.debug(f"Found evdev device: {device.name} - {device.path} - {device.phys}")
    ...
    def received_button(self, device_name, event):
        ...
        value = event.value
        self.logger.debug(f'Button press request: Device: {device_name}; Keycode: {data.keycode}; Value: {value})')
        ...
        if value:
            for button in self.config['nodes'][self.core.hostname][device_name].keys():
                if button == data.keycode:
                    ...
                    self.logger.info(f'Evdev button command ({command})')
        ...
        try:
            for device_name in sorted(self.config['nodes'][self.core.hostname]):
                for button in sorted(self.config['nodes'][self.core.hostname][device_name].keys()):
                    command = self.config['nodes'][self.core.hostname][device_name][button]
                    ret.append(f'{device_name:<15} {button:<15} {command}')
        except TypeError:
            pass
        return ret
    ...
    def cmd_list_devices(self, args):
        ret = []
        for name, path, phys in self.get_all_devices():
            ret.append(f'{name:<25} {path:<15} {phys}')
        return ret
